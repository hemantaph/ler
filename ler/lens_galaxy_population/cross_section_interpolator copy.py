...
@njit(cache=True)
def _physical_to_index(x, grid):
    """
    Convert a physical coordinate to a fractional grid index.

    Supports nonuniform monotonic grids by locating the bracketing interval
    and linearly interpolating the local index.
...
def _cross_section(
    ...
    for i in range(N):
        ie1[i] = _physical_to_index(e1[i], e1_grid)
        ie2[i] = _physical_to_index(e2[i], e2_grid)
        ig[i] = _physical_to_index(gamma[i], gamma_grid)
        ig1[i] = _physical_to_index(gamma1[i], gamma1_grid)
        ig2[i] = _physical_to_index(gamma2[i], gamma2_grid)
