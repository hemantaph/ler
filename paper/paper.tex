...
\fancyfoot[L]{\parbox[t]{0.98\headwidth}{\footnotesize{\sffamily D. Keitel, R. Tenorio, G. Ashton, R. Prix, (2021). \(ler\)
: LVK (LIGO-Virgo-KAGRA collaboration) Event (compact-binary mergers)
Rate calculator and
simulator. \textit{Journal of Open Source Software}, 6(60), 3000. \url{https://doi.org/10.21105/joss.03000}}}}
...
Science Park, 1098 XG Amsterdam, The Netherlands}
      \affil[6]{Departament de F\textquotesingle isica, Universitat de
...
library, enhanced by interpolation methods from \emph{scipy} and
Python's \emph{multiprocessing} capabilities. Efficiency is further
boosted by the \emph{numba} library's Just-In-Time (\emph{njit})
...
\item
  ...
  \item
    The process of sampling both intrinsic and extrinsic parameters of
    GW sources, represented by \(\theta\), utilizes the prior
    distributions \((P(\theta))\) available within the \emph{gwcosmo}
    and \emph{bilby} Python packages. Additionally, users have the
    option to initialize the package with custom distributions of their
    choosing.
  \end{itemize}
\item
  ...
  \item
    ...
    \((\text{optical depth: }P\left(\text{SL}|z_s\right)\,)\). This
    probability can be recalculated for specified configurations of lens
    galaxies, leveraging \emph{multiprocessing} and \emph{njit}
  ...
  \item
    ...
    \((\theta_L)\) sampling, following reference
    (https://arxiv.org/abs/2106.06303)(Wierda et al. 2021). Rejection
...
\item
  ...
  \item
    ...
    \emph{lenstronomy} to generate properties of the images. This is the
    slowest part of the entire simulation, which \(ler\) tackles through
...
\item
  ...
  \item
    ...
    \href{https://github.com/hemantaph/gwsnr}{\emph{gwsnr}}, leveraging
    interpolation and multiprocessing for accuracy and speed.
  \item
